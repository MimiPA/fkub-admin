...
export default function DokumenPendukungList({ id_pengajuan }) {
    ...
    const columns = [
        ...
        {
            name: "Telepon",
            selector: (row) => row.Pengguna.telepon,
            sortable: true,
        },
        {
            name: "Jenis Kelamin",
            selector: (row) => row.Pengguna.jenis_kelamin,
            sortable: true,
        },
        {
        ...
        {
            name: "Surat Dukungan",
            selector: (row) => row.surat_pernyataan,
            cell: (row) => (
                <Link href={`${row.surat_pernyataan}`}>
                    <button type="button" onClick={`window.open(${row.surat_pernyataan}, '_blank')`} >
        ...
        {
            name: "Foto KTP",
            selector: (row) => row.foto_ktp,
            cell: (row) => (
                <Link href={`${row.foto_ktp}`}>
                    <button type="button" onClick={`window.open(${row.foto_ktp}, '_blank')`} >
        ...
        {
            name: "Foto Diri",
            selector: (row) => row.foto_diri,
            cell: (row) => (
                <Link href={`${row.foto_diri}`}>
                    <button type="button" onClick={`window.open(${row.foto_diri}, '_blank')`} >
    ...
    useEffect(() => {
        api.get(`/proposal/user/list/detailPendukung/${id_pengajuan}`).then(res => {
                                ...
                                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                                    ...
                                    <div className="mt-4">
                                        <p className="text-sm text-gray-500">
                                            Apakah Anda Yakin Ingin Mengubah Status Pendukung User Berikut ? {activeRow.id}
                                        </p>
                                    </div>
