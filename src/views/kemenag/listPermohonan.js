...
const columns = [
    ...
    {
        name: "Nama Pemohon",
        selector: (row) => row.Pengajuan.Pengguna.nama_depan + ' ' + row.Pengajuan.Pengguna.nama_belakang,
        sortable: true,
    },
    {
    ...
    {
        name: "Nama Tempat",
        selector: (row) => row.Pengajuan.nama_tempat,
        sortable: true,
    },
    {
        name: "Jenis Pembangunan",
        selector: (row) => row.Pengajuan.jenis_pembangunan,
        sortable: true,
    },
    {
        name: "Alamat",
        selector: (row) => row.Pengajuan.alamat,
        sortable: true,
    },
    {
        ...
        cell: (row) => (
            <a target="_blank" href={`${row.dokumen}`}>
                <div>
    ...
    },
    {
        button: true,
        cell: (row) => (
            <Link href={`/kemenag/daftar_permohonan/${row.id}`}>
...
export default function ListPermohonan() {
    ...
    useEffect(() => {
        api.get('/administrasi/kemenag/list').then(res => {
    ...
    const filteredItems = data.filter(item =>
        ...
        item.Pengajuan.Pengguna.nama_depan && item.Pengajuan.Pengguna.nama_depan.toLowerCase().includes(filterText.toLowerCase()) ||
    ...
    return (
        <>
            <div>
                <DataTable
                    title="Daftar Permohonan Surat Rekomendasi Kemenag"
                    columns={columns}
                    data={filteredItems}
                    subHeader
